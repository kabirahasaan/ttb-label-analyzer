...
import { typography, spacing, stepIndicators } from '@/styles/page-consistency';
...
export default function UploadLabelPage() {
  ...
  return (
    <main className={`bg-white ${spacing.pageVertical}`} aria-live="polite">
      <div className={spacing.pageContainer}>
        <div className={spacing.headerSection}>
          <div className="text-center">
            <h1 className={typography.pageTitle}>Validate Label</h1>
        ...
        <div className={stepIndicators.container}>
          <div key={1} className="flex items-center">
            <div
              className={`${completedSteps.upload ? stepIndicators.step.complete : 1 === 1 && !completedSteps.upload ? stepIndicators.step.active : stepIndicators.step.inactive}`}
            >
              {completedSteps.upload ? <CheckCircle2 className="h-5 w-5" /> : '1'}
            </div>
            <ChevronRight className="mx-2 h-5 w-5 text-gray-400 hidden sm:block" />
          </div>
          <div key={2} className="flex items-center">
            <div
              className={`${completedSteps.application ? stepIndicators.step.complete : completedSteps.upload ? stepIndicators.step.active : stepIndicators.step.inactive}`}
            >
              {completedSteps.application ? <CheckCircle2 className="h-5 w-5" /> : '2'}
            </div>
            <ChevronRight className="mx-2 h-5 w-5 text-gray-400 hidden sm:block" />
          </div>
          <div key={3} className="flex items-center">
            <div
              className={`${completedSteps.validation ? stepIndicators.step.complete : completedSteps.application ? stepIndicators.step.active : stepIndicators.step.inactive}`}
            >
              {completedSteps.validation ? <CheckCircle2 className="h-5 w-5" /> : '3'}
            </div>
          </div>
        </div>
                ...
                >
                  ...
                  {applications.map((app) => (
                    <option key={app.id} value={app.id}>
                      {app.colaNumber || 'No COLA ID'} · {app.brandName} · {app.alcoholByVolume}%
                      ABV · {app.netContents} · {app.producerName}
                    </option>
                  ))}
                </select>
              </div>
                    ...
                    >
                      ...
                      <div className="text-left">
                        ...
                        <div className="text-xs text-slate-600">
                          Reset and validate another label
                        </div>
                      </div>
                    </Button>
                  </div>
